...
public:
	...
	void SetUseDmg(bool dmg) { m_bDmg = dmg; };					//ダメージ処理を無効にするかどうか
	void SetHitRig(int dmgrig) { m_nHitRig = dmgrig; };			//ダメージ硬直の設定
	void SetGuardRig(int dmgrig) { m_nGuardRig = dmgrig; };		//ガード硬直の設定
	void SetHitBack(int dmgback) { m_nHitBack = dmgback; };		//ヒットバックの設定
	void SetGuardBack(int dmgback) { m_nGuardBack = dmgback; };	//ガードバックの設定
	void SetDamage(int dmg)	 { m_nDamage = dmg; };				//ダメージ量設定
	...
	int				GetHitBack() { return m_nHitBack; };		//ヒットバックの設定
	int				GetGuardBack() { return m_nGuardBack; };	//ガードバックの設定
...
private:
	...
	int						m_nHitBack;		//ヒットした際の移動量
	int						m_nGuardBack;	//ガードした際の移動量
};
