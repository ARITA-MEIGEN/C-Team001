...
void CMotion::SetMotion(const int nCntMotionSet)
{
	...
	for (int nCntParts = 0; nCntParts < m_nMaxParts; nCntParts++)
	{
		...
		D3DXVECTOR3 posOrigin = m_parts[nCntParts]->GetDPos();	// 元の位置
		D3DXVECTOR3 rotOrigin = m_parts[nCntParts]->GetDRot();	// 元の向き
...
void CMotion::SetPartsOrigin()
{
	...
	{
		// 位置の設定
		m_parts[nCntParts]->SetPos(m_parts[nCntParts]->GetDPos());

		//	向きの設定
		m_parts[nCntParts]->SetRot(m_parts[nCntParts]->GetDRot());
	}
}
