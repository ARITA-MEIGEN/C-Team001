													...
													while (fgets(Read, lenLine, sta) != nullptr)
													{
														...
														else if (strcmp(&strLine[0], "COLLISIONSET") == 0)
														{
															...
															while (fgets(Read, lenLine, sta) != nullptr)
															{
																...
																else if ((strcmp(&strLine[0], "DAMAGE") == 0))
																{//ダメージ量
																	int Damage;
																	sscanf(Read, "%s = %d", &strLine, &Damage);	//キーの総数
																	m_apMotion[motionnumber].aKey[key].Collision[select]->SetDamage(Damage);
																}
																else if ((strcmp(&strLine[0], "HITBACK") == 0))
																{//ヒットバック
																	int move;
																	sscanf(Read, "%s = %d", &strLine, &move);	//キーの総数
																	m_apMotion[motionnumber].aKey[key].Collision[select]->SetHitBack(move);
																}
																else if ((strcmp(&strLine[0], "GUARDBACK") == 0))
																{//ガードバック
																	int move;
																	sscanf(Read, "%s = %d", &strLine, &move);	//キーの総数
																	m_apMotion[motionnumber].aKey[key].Collision[select]->SetGuardBack(move);
																}
															}
														}
...
void CPlayer::MotionPlayer()
{
	...
	{//キー番号の更新とカウンターのリセット
		...
		{//ダメージフラグ復活
			m_apMotion[m_Motion].aKey[m_nCurKey].Collision[j]->SetUseDmg(true);
		}
	}
	m_frame++;
}
...
bool CPlayer::Damage()
{
	...
	{//現在のモーションのやられ判定の数だけループ
		for (int j = 0; j < m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].nNumCollision; j++)
		{//現在の敵のモーションの当たり判定の数だけループ
			if (m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].Collision[j]->GetUseDmg() == true)
			{//敵のダメージ判定残ってるかどうか
				if (ColJudge(i, j) == true)	//矩形の判定
				{
					m_nHitStop = m_pEne->m_apMotion[m_pEne->m_Motion].nHitStopTimer;											//ヒットストップ設定
					m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].Collision[j]->SetUseDmg(false);					//ヒットしたダメージ判定をモーション再生終了まで削除
					if (Guard(m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].Collision[j]->GetProperty()) == false)
					{//ガードできていない場合
						Hit(m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].Collision[j]->GetDamage(),					//自身がダメージを食らった時の処理
					...
					{//ガード成功
						GuardSuccses(true);
						//ガード硬直の値を代入
						m_nRig = m_pEne->m_apMotion[m_pEne->m_Motion].aKey[m_pEne->m_nCurKey].Collision[j]->GetGuardRig();
					}
				}
			}
		}
...
bool CPlayer::Guard(CCollision::EDAMAGE_POINT dp)
{
	...
	switch (dp)
	{
	case CCollision::DP_HIGH:
		pos = D3DXVECTOR3{ -20.0f,30.0f,0.0f };			//位置の設定

		if (m_Motion == PM_ST_GUARD || m_Motion == PM_CR_GUARD)
		{
			m_bGuard = true;
		}
		break;
	case CCollision::DP_MIDDLE:
		pos = D3DXVECTOR3{ -20.0f,30.0f,0.0f };			//位置の設定

		if (m_Motion == PM_ST_GUARD)
		{
			m_bGuard = true;
		}
		break;
	case CCollision::DP_LOW:
		pos = D3DXVECTOR3{ -20.0f,20.0f,0.0f };
		if (m_Motion == PM_CR_GUARD)
		{
			m_bGuard = true;
		}
		break;
	...
	if (m_bGuard == true)
	{//ガードエフェクト
		CParticle::Create(m_pos + pos,			//位置の設定
...
{
	if (m_pEne->m_nLife >= 0 && m_bAttack == false)
	{//生きてて攻撃していない場合
		//移動
		if (m_State != PST_AIR)
		{
			...
			if ((m_anInput[0] & INPUT4) == INPUT4)
			{
				...
				case CPlayer::PST_STAND:
					...
					if (m_bLeftSide == false)
					{
						...
						{//弾の防御処理
							if (m_pEne->m_pBullet->GetPos().x - m_pos.x <= BULLET_GUARD&&m_pEne->m_pBullet->GetPos().x - m_pos.x >= -BULLET_GUARD)
							{
								m_Motion = PM_ST_GUARD;
								m_move = { 0.0f,0.0f,0.0f };
							}
			...
			{
				if (m_bLeftSide == true && m_Motion != PM_ST_DASH)
				{//右向きの場合
					//前ダッシュ
					m_Motion = PM_ST_DASH;
				}
				else if (m_bLeftSide == false && m_Motion != PM_ST_DASH)
				{//左向き(2P側)の場合
					//バックステップ
			...
			{
				if (m_bLeftSide == false && m_Motion != PM_ST_DASH)
				{//左向き(2P側)の場合
				 //前ダッシュ
					m_Motion = PM_ST_DASH;
				}
				else if (m_bLeftSide == true && m_Motion != PM_ST_DASH)
				{//右向きの場合
				 //バックステップ
					m_Motion = PM_ST_BACKSTEP;
					m_move = { -BACKSTEP_MOVE_X,BACKSTEP_MOVE_Y,0.0f };
			...
			if ((m_anInput[0] & INPUT2) == INPUT2)
			{
				if (m_State != PST_AIR)
				{
					...
					if ((m_bLeftSide == false && (m_anInput[0] & INPUT6) == INPUT6) || (m_bLeftSide == true && (m_anInput[0] & INPUT4) == INPUT4))
					{
						if (m_pEne->m_bAttack == true)
						{
			...
			if ((m_anInput[0] & INPUT8) == INPUT8)
			{
				...
				m_State = PST_AIR;
				m_bJump = true;
				m_nJumpTransTime = JUMP_TRANS_TIME;
			}
...
void CPlayer::StateManagement()
{
	...
	{//攻撃していない場合、地上時のみダッシュ維持
		if (m_bLeftSide==true && (m_anInput[0] & INPUT6) == INPUT6 && (m_anInput[0] & INPUT_NOT4) == INPUT_NOT4)
		{//前押しっぱなしでかつ後ろ入力なし
			m_Motion = PM_ST_DASH;
			m_move.x = DASH_SPEED;
		}
		else if (m_bLeftSide==false && (m_anInput[0] & INPUT4) == INPUT4 && (m_anInput[0] & INPUT_NOT6) == INPUT_NOT6)
		{
			m_Motion = PM_ST_DASH;
			m_move.x = -DASH_SPEED;
		}
...
void CPlayer::Cancel()
{
	...
	case CPlayer::PM_ST_HATTACK:
		m_NextMotion = FireBall(m_NextMotion);
...
//被弾時の処理時の処理
//==============================================
void CPlayer::Hit(int Damage, CCollision::EDAMAGE_POINT pro, int HitRig, bool knockback)
{
	m_nLife -= Damage;
...
//遠距離技
//==============================================
CPlayer::PLAYER_MOTION CPlayer::FireBall(PLAYER_MOTION nowmotion)
{
	D3DXVECTOR3 move;
	...
	{
		if (m_bLeftSide == true && m_bBullet == false)
		{
			motion = PM_236H;
			m_bBullet = true;
		}
	...
	{
		if (m_bLeftSide == false && m_bBullet == false)
		{
			motion = PM_236H;
			m_bBullet = true;
		}
	...
	{//弾発射モーションに応じて性能を変化させる
		...
		else if (motion == PM_236M)
		{
			move = D3DXVECTOR3(2.5f, 0.0f, 0.0f);
			life = 7;	//エフェクトの寿命
		}
		else if (motion == PM_236H)
		{
		...
		if (m_MotionOld != motion && (motion == PM_236L || motion == PM_236M || motion == PM_236H))
		{//弾のモーション再生中に弾を発射する
			m_bAttack = true;
...
//同時に更新するやつ
//==============================================
void CPlayer::UpdateAfter()
{
	
}

//ダメージモーションに移行
void CPlayer::TransDamageMotion()
{
	if (m_bDamage == true)
	{
		switch (m_State)
...
//被弾時のベクトル設定
void CPlayer::TransDamageVector()
{
	if (m_bDamage == true)
	{
